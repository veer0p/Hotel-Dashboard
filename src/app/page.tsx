...
import { Box, Typography, Paper, Button, Skeleton, useMediaQuery, useTheme } from "@mui/material";
...
import MobileRoomList from "@/components/dashboard/MobileRoomList";
...
export default function Home() {
  ...
  const handleRoomContextMenu = useCallback((event: React.MouseEvent<HTMLDivElement>, room: any) => {
  ...
  return (
    <MainLayout>
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" sx={{ fontWeight: 700, mb: 1, fontFamily: 'Inter, sans-serif' }}>
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 5, borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>Living Floor Plan</Typography>
          {!isMobile && (
        ...
        {isMobile ? (
          <MobileRoomList rooms={mappedRooms} onRoomClick={handleRoomClick} onRoomContextMenu={handleRoomContextMenu} />
        ) : (
          <Box sx={{ display: 'flex', gap: 2, overflowX: 'auto', pb: 2 }}>
            {mappedRooms.map((room) => (
              <RoomMiniCard key={room.roomNumber} room={room as any} onClick={handleRoomClick} onContextMenu={handleRoomContextMenu} />
            ))}
          </Box>
        )}
      </Paper>
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button variant="contained" startIcon={<UserPlus size={18} />} onClick={() => router.push('/checkin')} sx={{ textTransform: 'none', fontWeight: 600, borderRadius: 2 }}>
              Quick Check-in
            </Button>
            <Button variant="outlined" startIcon={<Calendar size={18} />} sx={{ textTransform: 'none', fontWeight: 600, borderRadius: 2 }}>
              New Booking
            </Button>
            <Button variant="outlined" startIcon={<List size={18} />} sx={{ textTransform: 'none', fontWeight: 600, borderRadius: 2 }}>
              View All
            </Button>
          </Box>
        </Box>
      ...
      <RoomDetailSlideOver open={selectedRoom !== null} onClose={() => setSelectedRoom(null)} room={selectedRoom} />
      <RoomContextMenu
        anchorEl={contextMenu ? { getBoundingClientRect: () => ({ top: contextMenu.mouseY, left: contextMenu.mouseX, right: contextMenu.mouseX, bottom: contextMenu.mouseY, width: 0, height: 0, x: contextMenu.mouseX, y: contextMenu.mouseY, toJSON: () => ({}) }) } as any : null}
